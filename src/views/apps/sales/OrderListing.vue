<template>
  ...
  <div class="card">
    ...
    <div class="card-header border-0 pt-6">
      ...
      <div class="card-title">
        ...
        <div class="d-flex align-items-center position-relative my-1">
          ...
          <input v-model="search" @input="searchItems()" type="text" data-kt-subscription-table-filter="search"
            class="form-control form-control-solid w-250px ps-14" placeholder="Search Orders" />
        </div>
        <!--end::Search-->
        <!--begin::Status Search-->
        <div class="d-flex align-items-center position-relative my-1 px-4">
          <select v-model="selectedState" @change="searchStatus()" class="form-select">
      ...
      <div class="filters d-flex position-relative my-1 px-1">
        <div class="date-filter">
          <label>
            Fecha inicial:
            <input class="form-control form-control-solid w-250px" type="date" v-model="startDate" />
          </label>
          <label class="px-4">
            Fecha final:
            <input class="form-control form-control-solid w-250px" type="date" v-model="endDate" />
          </label>
        </div>
      </div>
      <!--begin::Card toolbar-->
      <div class="card-toolbar">
        ...
        <div v-else class="d-flex justify-content-end align-items-center">
          ...
          <button type="button" class="btn btn-danger" @click="deleteFewSales()">
            Delete Selected
          </button>
        </div>
        <!--end::Group actions-->
    ...
    <div class="card-body pt-0">
      <KTDatatable @on-sort="sort" @on-items-select="onItemSelect" @on-items-per-page-change="onItemsPerPageChange"
        :data="data" :header="headerConfig" :checkbox-enabled="true" :items-per-page="10">
        ...
        <template v-slot:actions="{ row: customer }">
          ...
          <div
            ...
            <div class="menu-item px-3">
              <a @click="deleteSale(customer.id)" class="menu-link px-3">Delete</a>
            </div>
            <!--end::Menu item-->
          </div>
          <!--end::Menu-->
            ...
            <div class="modal-body">
              <div>
                <div v-if="isEditable">
                  ...
                  <p>
                    Nombre:
                    <input type="text" v-model="selectedOrder!.customer" @blur="updateOrder(selectedOrder)"
                      class="form-control form-control-solid w-250px" aria-label="Edit customer name" />
                  </p>
                  <p>
                    Estado:
                    <select v-model="selectedOrder!.status"
                      @change="updateOrder(selectedOrder), assignColorByStatus(selectedOrder!)"
                      class="form-control form-control-solid w-250px" aria-label="Edit status">
                      <option value="">All</option>
                      <option v-for="estado in estados" :key="estado" :value="estado">
                        {{ estado }}
                      </option>
                  ...
                  <p>
                    Total:
                    <span>$</span>
                    <input type="number" v-model="selectedOrder!.total" @blur="updateOrder(selectedOrder)"
                      class="form-control form-control-solid w-250px" aria-label="Edit total" step="0.01" />
                  </p>
                  <p>
                    Date Added:
                    <input type="date" v-model="selectedOrder!.dateadded" @blur="updateOrder(selectedOrder)"
                      aria-label="Edit dateadded" class="form-control form-control-solid w-250px"
                      @focus="isEditingDateAdded = true">
                  </p>
                  <p>
                    Created Date:
                    <input type="date" v-model="selectedOrder!.createdDate" @blur="updateOrder(selectedOrder)"
